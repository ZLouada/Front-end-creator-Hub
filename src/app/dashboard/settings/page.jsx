...
function Section({ title, subtitle, children }) {
  return (
    <div className="bg-surface-card rounded-2xl shadow-soft border border-brand-300/20 overflow-hidden">
      <div className="px-6 py-5 border-b border-brand-300/20">
    ...
    <div className="min-h-full bg-surface p-5 sm:p-8">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="animate-fade-up">
          ...
          <p className="text-gray-500 font-medium mt-1 text-sm">Manage your account and billing preferences</p>
        </div>
            ...
            <form onSubmit={handleSave} className="space-y-4 max-w-md">
              <Field label="Display Name">
                <input
                  ...
                  className="input-modern w-full px-4 py-3 bg-brand-50 border-2 border-brand-300/20 rounded-xl text-gray-800 font-semibold text-sm placeholder-gray-400"
                />
              </Field>
              <Field label="Email Address" hint="Contact support to change your email.">
                <input
                  ...
                  className="w-full px-4 py-3 bg-brand-100 border-2 border-transparent rounded-xl text-gray-400 font-semibold text-sm cursor-not-allowed"
                />
              </Field>
              <button
                type="submit"
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all hover:-translate-y-0.5 hover:shadow-soft-md active:scale-95 ${
                  saved
                    ? 'bg-success text-white'
                    : 'bg-gradient-to-br from-brand-400 to-brand-500 text-brand-900 shadow-glow-sm'
                }`}
              >
                {saved ? (
                  <>
          ...
          <Section title="Payment History" subtitle="All your past transactions">
            <div className="overflow-x-auto -mx-2">
              <table className="w-full text-left min-w-[480px]">
                <thead>
                  <tr className="text-[10px] font-bold text-gray-400 uppercase tracking-widest border-b border-brand-300/20">
                ...
                <tbody className="divide-y divide-gray-50">
                  {paymentHistory.map((row) => (
                    <tr key={row.date} className="hover:bg-brand-50/60 transition-colors">
                      <td className="py-3.5 pl-2 text-sm text-gray-500 font-medium">{row.date}</td>
                      <td className="py-3.5 text-sm text-gray-700 font-semibold">{row.desc}</td>
                      <td className="py-3.5 text-sm text-gray-900 font-bold">{row.amount}</td>
                      <td className="py-3.5">
                      ...
                      <td className="py-3.5 text-right pr-2">
                        <button className="text-xs font-bold text-brand-600 hover:text-brand-700 transition-colors">↓ PDF</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="animate-fade-up" style={{ animationDelay: '0.22s' }}>
          <Section title="Danger Zone" subtitle="Irreversible actions — proceed with caution">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                <p className="text-sm font-bold text-gray-700">Delete Account</p>
                <p className="text-xs text-gray-400 font-medium mt-0.5">Permanently remove your account and all data</p>
              </div>
              <button className="px-4 py-2.5 rounded-xl text-sm font-bold text-danger border-2 border-danger-200 hover:bg-danger-50 transition-all active:scale-95">
                Delete Account
              </button>
            </div>
          </Section>
    ...
    <div className="flex items-center justify-between">
      ...
      <button
        ...
        className={`relative shrink-0 w-11 h-6 rounded-full transition-all duration-300 ${
          on ? 'bg-brand-400 shadow-glow-sm' : 'bg-gray-300'
        }`}
      >
        <span
