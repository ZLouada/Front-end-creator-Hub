    ...
    <div className="min-h-full bg-[#F8F8F8] p-5 sm:p-8">
      <div className="max-w-3xl mx-auto space-y-6">
            ...
            <form onSubmit={handleSave} className="space-y-4 max-w-md">
              <Field label="Display Name">
                <input
                  ...
                  className="input-modern w-full px-4 py-3 bg-gray-50 border-2 border-gray-100 rounded-xl text-gray-800 font-semibold text-sm placeholder-gray-400"
                />
              </Field>
              <Field label="Email Address" hint="Contact support to change your email.">
                <input
                  ...
                  className="w-full px-4 py-3 bg-gray-100 border-2 border-transparent rounded-xl text-gray-400 font-semibold text-sm cursor-not-allowed"
                />
              </Field>
              <button
                type="submit"
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-gray-900 transition-all hover:-translate-y-0.5 hover:shadow-md active:scale-95"
                style={{ background: saved ? '#10b981' : 'linear-gradient(135deg,#FFDD00,#FFB300)', color: saved ? '#fff' : '#111' }}
              >
                {saved ? (
                  <>
        ...
        <div className="animate-fade-up" style={{ animationDelay: '0.14s' }}>
          <div className="mb-3">
            <h3 className="text-base font-black text-gray-900">Active Subscription</h3>
          </div>
          <SubscriptionManager />
        </div>
                ...
                <tbody className="divide-y divide-gray-50">
                  {paymentHistory.map((row) => (
                    <tr key={row.date} className="hover:bg-gray-50/60 transition-colors">
                      ...
                      <td className="py-3.5 text-sm text-gray-900 font-black">{row.amount}</td>
                      <td className="py-3.5">
                      ...
                      <td className="py-3.5 text-right pr-2">
                        <button className="text-xs font-bold text-yellow-600 hover:text-yellow-700 transition-colors">↓ PDF</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="animate-fade-up" style={{ animationDelay: '0.22s' }}>
          <Section title="Danger Zone" subtitle="Irreversible actions — proceed with caution">
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <button className="px-4 py-2.5 rounded-xl text-sm font-bold text-red-600 border-2 border-red-200 hover:bg-red-50 transition-all active:scale-95">
                Delete Account
              </button>
            </div>
          </Section>
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <button
        ...
        style={{ background: on ? '#FFDD00' : '#e5e7eb' }}
      >
        <span
