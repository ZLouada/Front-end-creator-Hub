...
function StatCard({ icon, label, value, suffix = '', badge, badgeColor, gradientFrom, gradientTo, delay = 0 }) {
  return (
    <div
      className="relative rounded-2xl p-6 overflow-hidden animate-fade-up"
      style={{
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <p className="text-white text-3xl font-black tracking-tight">{suffix === '$' ? `$${value.toLocaleString()}` : `${value}${suffix}`}</p>
        {badge && (
...
export default function AnalyticsDashboard() {
  ...
  return (
    <div className="min-h-full bg-[#F8F8F8] p-5 sm:p-8">
      <div className="max-w-6xl mx-auto space-y-8">

        <div className="flex items-center justify-between animate-fade-up">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight">Analytics</h1>
            <p className="text-gray-500 font-medium mt-1 text-sm">Your creator dashboard — last 6 months</p>
          </div>
          <button
            className="hidden sm:flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-gray-900 transition-all hover:-translate-y-0.5 hover:shadow-lg active:scale-95"
            style={{ background: 'linear-gradient(135deg,#FFDD00,#FFB300)', boxShadow: '0 4px 14px rgba(255,179,0,0.35)' }}
          >
            <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            Export CSV
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          ...
          <StatCard
            ...
            gradientFrom="#FFDD00" gradientTo="#FFB300" delay={0.08}
          />
          <StatCard
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-fade-up" style={{ animationDelay: '0.2s' }}>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-base font-black text-gray-900">Revenue Growth</h3>
                <p className="text-xs text-gray-400 font-medium mt-0.5">vs previous period</p>
              </div>
              <span className="text-xs font-bold px-3 py-1.5 rounded-xl bg-yellow-50 text-yellow-700 border border-yellow-100">
                6 months
              </span>
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={revenueData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                  ...
                  <Area type="monotone" dataKey="revenue" stroke="#FFDD00" strokeWidth={3} fill="url(#rev)" dot={{ r: 4, fill: '#FFDD00', strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 6, strokeWidth: 3, stroke: '#fff' }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>

          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-fade-up" style={{ animationDelay: '0.26s' }}>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-base font-black text-gray-900">New Supporters</h3>
                <p className="text-xs text-gray-400 font-medium mt-0.5">Monthly signups</p>
              </div>
              <span className="text-xs font-bold px-3 py-1.5 rounded-xl bg-gray-900 text-white">
                +42% avg
              </span>
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={subscriberData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                  ...
                  <Bar dataKey="subs" fill="#111827" radius={[8, 8, 2, 2]} barSize={36} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-fade-up" style={{ animationDelay: '0.32s' }}>
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            <h3 className="text-base font-black text-gray-900">Recent Activity</h3>
            <button className="text-xs font-bold text-yellow-600 hover:text-yellow-700 transition-colors">View all →</button>
          </div>
          <div className="divide-y divide-gray-50">
            {recentActivity.map((item, i) => (
              <div key={item.id} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50/60 transition-colors">
