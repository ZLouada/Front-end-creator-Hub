				...
				{gatewayOptions.map((gateway) => {
					...
					return (
						<button
							...
							className={`w-full text-left border rounded-xl p-4 transition-colors shadow-soft bg-surface-card hover:border-brand-500 hover:shadow-soft-md ${
								isSelected ? 'border-brand-600 ring-1 ring-brand-200' : 'border-brand-300/20'
							}`}
						>
							<div className="flex items-start gap-3">
								<div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold ${
									isSelected ? 'bg-brand-600 text-white' : 'bg-brand-100 text-gray-700'
								}`}>
									{gateway.label[0]}
								</div>
								<div className="flex-1">
									<div className="flex items-center gap-2">
										...
										{gateway.badge && (
											<span className="text-[10px] px-2 py-0.5 rounded-full bg-brand-50 text-brand-700 border border-brand-100 uppercase tracking-wide">
												{gateway.badge}
											</span>
										)}
									</div>
